...
def convert_to_jnp(obj):
    if isinstance(obj, torch.Tensor):
        return jnp.array(torch.Tensor.numpy(obj))
    elif isinstance(obj, int):
        return jnp.array(obj)
    elif isinstance(obj, (torch_geometric.data.storage.NodeStorage, torch_geometric.data.storage.EdgeStorage, torch_geometric.data.storage.BaseStorage)):
        for key in obj.keys():
            setattr(obj, key, convert_to_jnp(getattr(obj, key)))
        return obj
    elif isinstance(obj, dict):
        return {key: convert_to_jnp(value) for key, value in obj.items()}

    elif isinstance(obj, list):
        return [convert_to_jnp(item) for item in obj]
...
class InferenceDataset(Dataset):
    ...
    def get(self, idx):
        ...
        for attr, value in vars(complex_graph).items():
            # setattr(complex_graph, attr, convert_to_numpy(value))
            print(attr, type(value), value)

        if hasattr(complex_graph, '_global_store'):
            complex_graph._global_store = convert_to_jnp(complex_graph._global_store)

        # Convert tensors within the _node_store_dict
        if hasattr(complex_graph, '_node_store_dict'):
            for key in complex_graph._node_store_dict:
                complex_graph._node_store_dict[key] = convert_to_jnp(complex_graph._node_store_dict[key])

        if hasattr(complex_graph, '_edge_store_dict'):
            for key in complex_graph._edge_store_dict:
                complex_graph._edge_store_dict[key] = convert_to_jnp(complex_graph._edge_store_dict[key])
